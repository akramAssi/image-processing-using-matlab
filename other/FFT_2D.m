...
% 频域滤波步骤过程的图像
figure;
subplot(2,3,1);imshow(uint8(img));title('图(a):输入原图像');
subplot(2,3,2);imshow(uint8(img_fill));title('图(b):填充2M×2N的图像');
subplot(2,3,3);imshow(uint8(img_shift));title('图(c):乘以(-1)的(x+y)次方');
subplot(2,3,4);imshow(uint8(F_img));title('图(d):傅里叶变换频谱');
subplot(2,3,5);imshow(uint8(abs(H)),[]);title('图(e):实矩阵乘以一个复数矩阵');
